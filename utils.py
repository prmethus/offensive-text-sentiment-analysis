...
def download_and_join_datasets():
    ...
    if not os.path.exists(
        os.path.join(datasets_dir, "Cyberbully_dataset_combined.csv")
    ):
        ...
        dataframe = pd.concat(
            list(
                map(
                    lambda dataset: pd.read_csv(dataset)[["Text", "oh_label"]], datasets
                )
            ),
            axis=0,
        )
        ...
        dataframe.to_csv(
            os.path.join(datasets_dir, "Cyberbully_dataset_combined.csv"), index=False
        )
        print(
            r"Saved 'Cyberbully_dataset_combined.csv'. The dataset is also availabe on Kaggle: https://www.kaggle.com/datasets/prmethus/mendeleys-cyberbully-datasets-combined?select=Cyberbully_dataset_combined.csv"
        )
...
def tokenize_and_pad(texts, tokenizer, maxlen=100, padding="post", truncating="post"):
    ...
    padded_sequences = pad_sequences(
        sequences, maxlen, padding=padding, truncating=truncating
    )
...
def stratified_split(dataset):
    ...
    for train_set_index, validation_test_set_index in split1.split(
        dataset["Text"], dataset["Offensive"]
    ):
        train_set = dataset.loc[train_set_index, ["Text", "Offensive"]]
        validation_test_set = dataset.loc[
            validation_test_set_index, ["Text", "Offensive"]
        ]

    validation_test_set = validation_test_set.reset_index()

    for validation_set_index, test_set_index in split2.split(
        validation_test_set["Text"], validation_test_set["Offensive"]
    ):
        validation_set = validation_test_set.loc[
            validation_set_index, ["Text", "Offensive"]
        ]
